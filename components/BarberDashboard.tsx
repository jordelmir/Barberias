...
export const BarberDashboard: React.FC<BarberDashboardProps> = ({
    ...
    // --- REFINED HISTORICAL DATA & STATS ---
    const historicalStats = useMemo(() => {
        const filtered = myAllAppointments.filter(a => {
            ...
            switch (historyPeriod) {
                ...
                case 'yesterday': {
                    ...
                    return aptDate.getTime() >= yesterday.getTime() && aptDate.getTime() < today.getTime();
                }
                case 'week': {
                    const weekAgo = new Date(today);
                    weekAgo.setDate(weekAgo.getDate() - 7);
                    return aptDate.getTime() >= weekAgo.getTime();
                }
                case 'month':
        ...
        const revenue = filtered.reduce((sum, a) => sum + a.price, 0);
        ...
        filtered.forEach(a => {
            if (a.actualStartTime && a.actualEndTime) {
                totalMins += (a.actualEndTime.getTime() - a.actualStartTime.getTime()) / 60000;
                cutsWithTime++;
            }
        });
        const avgMins = cutsWithTime > 0 ? Math.round(totalMins / cutsWithTime) : 0;

        return { list: filtered, revenue, count, avgMins };
    }, [myAllAppointments, historyPeriod, now]);
            ...
            {viewMode === 'OPERATIONAL' && (
                <>
                                                ...
                                                <div key={apt.id} className={`group relative overflow-hidden rounded-xl border transition-all duration-300 ${isNext ? 'bg-brand-500/5 border-brand-500/30' : 'bg-dark-900/30 border-white/5 hover:border-white/10'}`}>
                                                    <div className="p-4 flex flex-col gap-3">
                                                        ...
                                                        <div className="flex gap-2 mt-1">
                                                            {activeCut ? (
                                                                <div className="w-full py-4 text-center text-[10px] text-gray-600 font-black uppercase tracking-widest border border-dashed border-gray-800 rounded-xl bg-dark-900/20">ESPERANDO TURNO</div>
                                                            ) : (
                                                                <>
                                                                    {!isCheckedIn ? (
                                                                        <button onClick={() => onStatusChange(apt.id, AppointmentStatus.CHECKED_IN)} className="flex-1 bg-dark-800 hover:bg-dark-700 text-gray-300 border border-white/10 rounded-xl py-4 text-[11px] font-black uppercase tracking-wider flex items-center justify-center gap-2 transition-all active:scale-95"><LogIn size={16} /> Check In</button>
                                                                    ) : (
                                                                        <button onClick={() => onStatusChange(apt.id, AppointmentStatus.IN_PROGRESS)} className="flex-1 bg-brand-500 hover:bg-brand-400 text-black border border-brand-400 rounded-xl py-5 text-[11px] font-black uppercase tracking-wider flex items-center justify-center gap-2 transition-all shadow-[0_0_20px_rgba(240,180,41,0.3)] animate-pulse-slow active:scale-95"><Play size={18} fill="black" /> Comenzar Corte</button>
                                                                    )}
                                                                    {!isCheckedIn && <button onClick={() => onStatusChange(apt.id, AppointmentStatus.CANCELLED)} className="w-14 bg-dark-800 hover:bg-red-900/20 hover:border-red-500/50 border border-white/10 rounded-xl flex items-center justify-center text-gray-500 hover:text-red-500 transition-all active:scale-95"><XCircle size={20} /></button>}
                                                                </>
                                                            )}
                                                        </div>
                                                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                        ...
                        <div className="lg:col-span-6 glass-morphism rounded-3xl overflow-hidden border-brand-500/10 min-h-[400px] flex flex-col">
                            <div className="p-5 border-b border-white/10 bg-dark-900/50 backdrop-blur space-y-4">
                                    ...
                                    <div className="flex bg-dark-800 rounded-lg p-1 border border-white/5">
                                        {(['today', 'yesterday', 'week', 'month'] as const).map((p) => (
                                            <button
                                                ...
                                                className={`px-3 py-1 rounded-md text-[9px] font-bold uppercase transition-all ${historyPeriod === p ? 'bg-brand-500 text-black shadow-lg shadow-brand-500/20' : 'text-gray-500 hover:text-gray-300'}`}
                                            >
                                ...
                                <div className="grid grid-cols-3 gap-3">
                                    <div className="bg-dark-900/50 border border-white/5 rounded-xl p-3 flex flex-col items-center justify-center">
                                        <span className="text-[10px] text-gray-500 font-bold uppercase tracking-tighter mb-1">Recaudado</span>
                                        <span className="text-sm font-black text-brand-500">${historicalStats.revenue.toLocaleString()}</span>
                                    </div>
                                    <div className="bg-dark-900/50 border border-white/5 rounded-xl p-3 flex flex-col items-center justify-center">
                                        <span className="text-[10px] text-gray-500 font-bold uppercase tracking-tighter mb-1">Servicios</span>
                                        <span className="text-sm font-black text-white">{historicalStats.count}</span>
                                    </div>
                                    <div className="bg-dark-900/50 border border-white/5 rounded-xl p-3 flex flex-col items-center justify-center">
                                        <span className="text-[10px] text-gray-500 font-bold uppercase tracking-tighter mb-1">Promedio</span>
                                        <span className="text-sm font-black text-gray-300">{historicalStats.avgMins}m</span>
                                    </div>
                                </div>
                            </div>

                            <div className="flex-1 overflow-y-auto custom-scrollbar p-0">
                                {historicalStats.list.length === 0 ? (
                                    <div className="h-full flex flex-col items-center justify-center text-gray-600 opacity-50 py-10">
                                        ...
                                        <p className="text-xs font-mono uppercase text-center px-4">Sin registros en este periodo</p>
                                    </div>
                                ) : (
                            ...
                            <div className="glass-morphism p-5 rounded-3xl flex-1 border-brand-500/10 relative">
                                ...
                                <div className="h-40 w-full">
                                    ...
                                    <div className="mt-4 space-y-2">
                                        {metricsXP.leaderboardData.slice(0, 3).map((entry, idx) => {
                                            const barberObj = barbers.find(b => b.id === entry.id);
                                            return (
                                                <div key={entry.id} className={`flex items-center justify-between p-2 rounded-xl border ${entry.isCurrent ? 'bg-brand-500/10 border-brand-500/30' : 'bg-dark-900/50 border-white/5'}`}>
                                                    <div className="flex items-center gap-3">
                                                        <div className={`w-6 h-6 rounded-full flex items-center justify-center font-bold text-[10px] ${idx === 0 ? 'bg-yellow-500 text-black' : idx === 1 ? 'bg-gray-400 text-black' : 'bg-amber-600 text-black'}`}>
                                                            {idx + 1}
                                                        </div>
                                                    ...
                                                    <span className="text-xs font-mono font-black text-brand-500">{entry.points} XP</span>
                                                </div>
                                            );
                                        })}
                                    </div>
